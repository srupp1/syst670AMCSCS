...
%
% Analytical values:
%   Detection recall   = cfg.detect_prob * weather  (Bernoulli mean)
...
WEATHER       = 1.0;
p_det_expect  = cfg.detect_prob * WEATHER;           % 0.95
...
fprintf('  Detection probability : %.4f (detect_prob=%.3f × weather=%.2f)\n', ...
    p_det_expect, cfg.detect_prob, WEATHER);
...
for call = 1:N_CALLS
    perc = computePerception(state, cfg, call);
    for j = 1:numel(perc)
